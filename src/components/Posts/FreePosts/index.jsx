...
function FreePosts() {
  ...
  // 자유 게시글 데이터 조회
  const { data: FreePosts, refetch } = useQuery(["freePosts", groupId], () => readFreePosts(groupId), {
  ...
  return (
    <Wrapper>
      ...
      <AllFreePost>
        <Scrollbars autoHide>
          {FreePosts && FreePosts.data.map((post) => <FreePostItem groupId={groupId} key={post.postId} refetch={refetch} post={post} />)}
        </Scrollbars>
      </AllFreePost>
    </Wrapper>
  );
